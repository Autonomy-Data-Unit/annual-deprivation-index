...
# 2. Check that at least 90% of each force-year's England-scope rows have an LSOA code
# 3. Load monthly street CSVs, excluding British Transport Police in every year
# 4. Remove exact duplicate identified records and all contributions from unusable force-years
# 5. Drop the remaining rows with missing LSOA codes and filter out Welsh LSOAs
# 6. Aggregate crime counts by LSOA and crime type
# 7. Merge with LSOA 2011 population data
# 8. Mark areas served by an unusable force-year unavailable (NaN), never as partial totals
# 9. Compute per-capita rates and save counts, rates, and population
#
